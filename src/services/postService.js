...
async function createPost(userId, content, files, username) {
  try {
    ...
    const userResponse = await axios.get(`${userServiceBaseUrl}/profile/${userId}`);
...
async function getUserPostByIdFromDb(userId) {
  ...
  let engagementData = {};
  try {
    const resp = await axios.post(`${engagementService}/counts`, { postIds });
